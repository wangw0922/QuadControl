...
## 三、现状基线（截至 2026-09-13，PR #24 已合并）
...
**2026-09-13（#24）**：P1 设备列表真数据、P1.5 会话监督库、P2 Android 一键
会话、P3 配对向导接真 adb 已合并，**M1 在代码层面达成**。但 P2/P3 的验证只在
Linux 虚拟机上用假 adb / 假 scrcpy 跑过，**P2 的 20 轮真机无孤儿验收与 P3 的
GUI 真机扫码端到端都尚未执行**，STATUS.md 如实标注。下一步是 P4。
...
### P5 手机端 App（**范围已重定，2026-08-12**）

原计划照设计交接稿列了「界面 D = iOS 被控端」「界面 E = Android 被控端」，
但与架构文档核对后发现**两者都与产品路径不符**。这是写计划时未回查架构文档
造成的疏漏，现更正：

- ~~**P5.1 iOS 被控端更新（界面 D）**~~：**取消**。`apps/ios` 属自研 Mac↔iPhone
  诊断栈，[ARCHITECTURE.md](ARCHITECTURE.md) 与
  [CONTROL_ARCHITECTURE.md](CONTROL_ARCHITECTURE.md) 都明确它**不在产品路径上**、
  **不得作为产品能力宣传**。产品里 iPhone 两格分别由 WDA（Win/Linux）与 Apple
  iPhone Mirroring（macOS）覆盖，**手机上不存在我们的 iOS App**。交接稿的界面 D
  描述了一个产品中不存在的组件。`apps/ios` 保留为连接诊断工具，不做产品化改造。

- **P5.2 Android 被控端 App**：**范围待定，需产品决策**。两条事实收窄了它：
  1. P3 已证实：安卓配对走**系统设置里的无线调试扫描器**，我们的 App 不参与，
     所以交接稿界面 E 的「扫码连接」主入口不成立；
  2. scrcpy 在手机上**不显示任何东西**。产品文档对可见性的论证是「用户自己开启
     开发者选项与 USB 调试并批准这台电脑」（授权可见），**不是**会话期间持续提示。

  因此这个 App 唯一有产品价值的职责是**把「会话期间可见」从授权可见升级为持续
  可见**：常驻前台通知「正在被电脑控制」+ 一键断开。**做不做是产品决策**——
  不做则维持现状（授权可见），界面文案必须与之一致（已于 2026-08-12 更正，
  见下）；做则需新建 Kotlin 工程并解决分发问题（Mode B 不进应用商店）。

  **相关修正**：界面 A 安全卡原文案「控制期间手机上会一直显示提示」对当前
  scrcpy 路径是**假的**，已改为陈述真实成立的保证（授权可见、随时可断开、
  结束恢复亮度/旋转/超时、不绕过锁屏）。
...
- 主线：P1 → P1.5 → P2 → P3 → P4 → （P5.2 视产品决策）→ P7，每片独立 PR、
  独立验收。P5.1 已取消（见上）。
- P6 与 CI-W 依赖外部条件（Windows 环境 / 9月配额），与主线并行，但 **P7 发布
  以 P6 完成为前提**（不发布未在目标平台真机验证过的安装包）。
- 里程碑：**M1 =“单机可用”**（P2 完成：macOS 上 Android 全流程）；
  **M2 =“四端打通”**（P4 完成：桌面端覆盖 Android 与 iPhone 两象限；手机侧
  按上文重定的范围，产品中本就不存在我们的 iOS App）；
