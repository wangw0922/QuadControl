...
- **Windows / Linux**：GUI 内渲染 WDA 的 MJPEG 画面；点击画面转发点按；文字转发卡；
  「唤醒屏幕」/ 截屏 / 回主屏三按钮；「已连接 · N 帧/秒」标签；「声音保留在手机上」
  说明卡；断开连接。
...
| MJPEG 参数 | `POST /session/{s}/appium/settings` `{"settings":{"mjpegServerFramerate":15,"mjpegServerScreenshotQuality":50}}` | 帧率只在编码侧限，与设备刷新率无关 |
| 窗口尺寸 | `GET /session/{s}/window/size` | 随 1 s 状态轮询刷新（旋转会变） |
| 点按 | `POST /session/{s}/actions`（W3C pointer） | 真机已验证；锁定时不转发 |
| 回主屏 | `POST /wda/homescreen` | 顶层端点，真机已验证 |
