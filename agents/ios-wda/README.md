...
| Gate 1 of P4.4 (2026-09-16, phone re-plugged first): probe `--serve` (CoreImage JPEG, quality 0.5) → existing proxy → GUI `<img>` in `LaunchMode::Attach` | probe: **58–60 fps capture on the home screen, JPEG encode 7.6–9.6 ms/frame, 50–100 KB/frame**; proxy took 54–60 parts/s; GUI label 58 fps. Render side (rAF + canvas pixel diff, changed frames per second): **50–56 in every second fully inside a 2 s home-screen drag**, median 32 across 8 drags (each 2 s of motion + 1.4 s idle). The Clock stopwatch page is not a valid motion source: the device sends only **30 fps** there (the app runs its display at 30 Hz) and the render count matched it 1:1. `requestAnimationFrame` stops entirely while the Mac is locked or the window is occluded, and the capture itself stalls to ~0.2 fps while the Mac is locked |
| Drag latency through the GUI's own session (2026-09-16) | `wda/dragfromtoforduration` with `duration` 2.0 returned after **3.4–3.5 s** each time (8 samples); the ~1.4 s overhead on top of the requested duration is the XCTest floor already seen for taps |
