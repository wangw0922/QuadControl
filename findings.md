...
## 2026-08-07 官方路径投屏：Mac 上第一次看到手机画面

- 用户问"能不能用安卓官方方式抓画面"——这个方向比内部 API 好得多，而我此前没有优先考虑，是个思路盲点。
- 实测官方 shell 工具（零内部 API）：`screencap` 单张 3088×1440 但 1.42 秒/张（约 0.7 fps，只适合偶尔截图）；`screenrecord --output-format=h264 -` 输出 **1280×596 @ 25 fps 有效 H.264**（ffprobe 确认）。
- **`screenrecord` 是真流式**：首字节 0.37 秒，之后持续约 500 KB/s，不是录完才吐。
- **`--time-limit 0` 官方支持无限时长**（默认 180 秒）。这条是关键：不加的话三分钟后画面直接冻住且无任何报错。
- 这基本绕开了"三星偏离 AOSP、每换机型要重新勘探"这个坏消息。据此把 Android 改为两层：官方命令打底（跨机型稳定），内部 API 仅作可选增强（降延迟、熄屏），探测不到就自动退回。
- Mac 端管线：Annex B 解析 → VideoToolbox 硬解 → CGImage → 窗口。实测 720×1600 持续镜像，Mac 端仅 4.6% CPU。
- **必须区分两个延迟**：实测的 0.37 秒是**启动延迟**（发命令到收到第一批字节），**不是端到端手感延迟**（还要加编码缓冲、传输、解码、绘制）。文档里已明确写死这条，不得混用。
- Annex B 解析必须跨 chunk 缓冲：管道会在任意位置切断，包括切在起始码中间。且一个 NAL 单元只有在**下一个**起始码出现后才算完整——提前吐出尾部会把截断的 slice 喂给解码器。测试覆盖了真实流前缀的每一个切分位置、逐字节投喂、以及三字节与四字节两种起始码（只认四字节会把两个单元静默合并成一个坏 slice）。
- Swift 6 严格并发不允许把 `CVImageBuffer` 跨 actor 传递。改成在解码队列完成 CIImage→CGImage 转换、用不可变的 `DecodedFrame` 过界——顺带把昂贵的色彩转换移出主线程。
- `CMBlockBufferCreateWithMemoryBlock` 不能直接拿 `Data` 的可变指针（编译器会警告 Data 可能含对象引用）。改为 `malloc` 一份拷贝并交给 `kCFAllocatorMalloc` 释放。
- 测试固件里的 SPS/PPS 取自真机，但那是纯编解码参数，不含任何画面内容；验证用的截图与视频流已全部删除，未进仓库。

## 2026-08-07 Mac 控制安卓：输入与旋转

- 输入延迟实测：`adb shell true` 38ms，每次新起 `adb` 的 `input keyevent` 103ms，**改走常驻 `adb shell` 后降到 53ms**。剩下约 51ms 是设备端为每个事件新起一个 `input` 进程，第一层绕不开。
- 因此手势尽量压成单条命令：拖拽用一条带时长的 `input swipe` 而非一串移动事件，字符串用一条 `input text` 而非逐字符。
- `input text` 实际只能发 ASCII（它把字符映射成按键事件），**中文输入在第一层不可用**。已做过滤并报告丢弃了几个字符，而不是让文字静默消失。
- **`dumpsys window displays` 的 `cur=` 是正确性关键**，不只是窗口尺寸问题：`wm size` 和 `init=` 报的是自然朝向，横屏时用它们会让每一次点击都被转置到错误位置。
- 旋转检测成本实测：`dumpsys window displays | grep -m1 cur=` **25ms**，`dumpsys input` 51ms，`settings get user_rotation` 67ms，`wm size` 54ms。选最便宜且唯一能报旋转后尺寸的那个，0.35 秒轮询一次。
- `screenrecord` 的帧尺寸启动后不能改，旋转必须重启流：**实测 800–965ms 才出第一帧新画面**。重启时必须同时重建解析器和解码器——新流有自己的参数集，把旧流的半个 NAL 单元带过去会让开头几帧花屏。
- 试过"正方形流 + 裁剪"来彻底消除重启：实测 `screenrecord` 确实保持比例居中留黑边（1280×1280 请求下 3088×1440 内容占 1280×597，53% 行是黑的），方案成立。但它浪费约一半编码面积，且窗口形状与手机不符，**用户明确否决**。改为保留重启并加 0.28 秒缓动动画、保持窗口中心不动、过渡期保留上一帧——同样的 850ms 感觉像过渡而不是卡死。
- 一个边界 bug：`CGRect.contains` 不含最大边，导致点击画面最右/最下一列被拒；且映射到 `width` 会超出有效像素（0…width-1）。已改为包含边界并把结果钳到最后一个像素。
- 菜单栏在 SwiftPM 可执行文件里不可靠（不是 `.app` bundle），Device 菜单根本没显示。改成窗口内按钮条，并设 `refusesFirstResponder`——否则点一次按钮后键盘焦点被抢走，打字就再也发不到手机。
- 坐标映射必须用**画面矩形**而非窗口矩形：`resizeAspect` 会居中留边，用窗口会让每次点击偏移半个黑边宽度。画面外的点击直接拒绝而不是钳到边缘——那会触发用户根本没瞄准的真实点击。

## 2026-08-07 Wi-Fi 无线连接与一次误诊排查

- **官方无线调试可用**：Android 11+ 的 `adb pair <ip:port> <码>` + `adb connect` 全程不需要 USB。`adb mdns services` 能自动发现配对服务（`_adb-tls-pairing._tcp`）和连接服务（`_adb-tls-connect._tcp`），省去用户手抄 IP。**这正好验证了 8 月 4 日 ADB probe 里那两个字段的实际用途**，当时只是理论上区分了它们。
- 连接后 `adb devices` 会出现**两个条目**（IP:端口 与 mDNS 服务名），是同一设备的两种寻址方式，后续命令必须显式指定 serial 否则报多设备错误。
- 无线 vs USB 延迟实测：shell 往返 44ms vs 2ms；`input keyevent` 97ms vs 53ms；旋转轮询 67ms vs 25ms。无线把常驻 shell 的优化基本吃掉了。
- **一次值得记录的误诊**：用户报告"画面延迟很高但操作延迟不高"。我依次怀疑并尝试了三个假设，**前两个都毫无效果**：
  1. 带宽不足（实测链路 0.5 MB/s，怀疑 8Mbps 超限）→ 降到 2M，无改善
  2. 解码器帧重排缓冲（`_EnableTemporalProcessing`）→ 去掉，无改善
  3. **真正原因：设备上累积了 4 个孤儿 `screenrecord` 进程**，互相抢占硬件编码器，把帧率饿到 0–4 fps
- 根因是我自己用 `pkill` 强杀 viewer —— `SIGKILL` 不会执行优雅退出的清理，设备端进程就留下了。**"降码率没效果"和"改解码器没效果"这两个否定结果才是最有价值的线索**，它们排除了带宽和解码路径，把矛头指向了别处；如果其中任何一个"碰巧有点改善"，我可能就停在错误的结论上了。
- Mac 侧统计 `decoded == drawn, 0 dropped` 是另一条关键证据：本地完全没有积压，帧是压根没来，而不是来了处理不过来。
- 已修：启动前和停止时都清扫设备上的 `screenrecord`。清理后同样条件下第一个窗口跑到 22 fps（此前 0–4 fps），用户确认"好很多"。
- 顺带修的两处仍然正确：解码器去掉帧重排（对实时镜像本就不该开，只是不是本次主因）；新增最新帧优先的合帧机制（慢链路下应该丢帧而不是让延迟无限增长）。
- 码率与分辨率改为按连接方式自动选择：USB 用 8M/1280，Wi-Fi 用 2M/800，均可命令行覆盖。

## 2026-08-09 扫码配对真机验收:三条链路全通,两个只有真机能暴露的问题

- **三条链路在 SM-S9180(Android 16 / adb 37.0.1-15733141)全部实测通过**:
  `pair-qr` 扫码 → 自动 pair+connect;`pair-code` 手动配对码;`connect` 显式重连。
  附带验证了无线镜像与控制(screenrecord H.264 → VideoToolbox → 窗口 + 输入)。
- **二维码反色,手机扫不出来——这是本轮最有价值的发现**。原先用 Unicode 半块
  字符渲染,深色主题终端下 `█` 用前景色(白)绘制,整张码黑白颠倒;标准二维码
  要求深色模块位于浅色背景,安卓设置里基于 zxing 的扫码器不处理反色码。
  **单测当时是通过的**:它把 `█` 当作黑模块去解码,解的是**语义矩阵**,不是
  屏幕上的**实际观感**。内容逐字节正确,可相机看到的是反的。
  改为 ANSI 背景色绘制(黑底=深色模块、白底=浅色、白色静区),不依赖终端主题;
  测试改为直接读 ANSI 背景色判定明暗,并断言静区必须为浅色。
  教训:**"内容正确"与"看起来正确"是两回事,而扫码器只看后者。**
- 整轮超时 90 秒不足以走完「设置 → 开发者选项 → 无线调试 → 使用二维码配对
  设备 → 扫码」,首次实测直接超时。改为 300 秒。这个值是拍脑袋定的,没有按
  真实操作路径估算过。
- 真机印证了两条此前只是纸上推演的设计:
  - USB 与无线两条 adb 条目经 `ro.serialno` 强身份去重后正确合并为 1 台可见设备;
  - 手动 `connect` 产生的是 **IP:端口**形式 serial(非 GUID 形式),方案 rev4
    预留的"无 GUID 时 ip:port 即主 serial"分支被真实触发。
- 配对端口与连接端口确实不同(实测 33231 vs 46875),验证了方案里"两组端口
  必须在 UI 上分开引导"的判断。
- 七轮 LLM 代码评审 + 54 项测试全绿,都没能发现上面两个问题;而 CI 首跑就抓到
  了一个只在 Windows 上发生的 PID 复用竞态。**不同性质的缺陷需要不同的裁判:
  评审看逻辑,CI 看跨平台,真机看物理世界。**
